...
How to run: python rbf_regression.py
...
#    img_plot(imgs)
    ...
    x_train_pca = perform_pca(x_train, 150, N, M)
    print('Shape of x_train after performing pca:', x_train_pca.shape)

    # perform PCA on x_train
    x_test_pca = perform_pca(x_test, 150, N, M)
    print('Shape of x_test after performing pca:', x_test_pca.shape)
    print("Pca took " + str(seconds) + ' seconds')
    
    # get the current time in seconds since the epoch
    seconds = time.process_time()

    mnist_net = RBF_Net(200, 10)
    mn_train_set = x_train_pca[:2000]
    y_train_set = y_train[:2000]

    mnist_net.train(mn_train_set, y_train_set)
    print("Training took " + str(seconds) + ' seconds')
    ...
    # test acc
    y_pred_test = mnist_net.predict(x_test_pca)
    acc_test = mnist_net.accuracy(y_test, y_pred_test)
    print('Accuracy on the testing set: {}% '.format(acc_test * 100))
    
def perform_pca(data, pcs_to_keep, N, M):
    # perform PCA before training RBF network

    # pandas DataFrame object that holds the data
    imgs = pd.DataFrame(data = data)
    
    # pca_cov object that holds the data_img DataFrame
    pca = pca_cov.PCA_COV(imgs)
    # run pca
    pca.pca(np.arange(M * M))
    return pca.pca_project(pcs_to_keep)
